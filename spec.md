...
- For each file, parses:
  - **Status**: from embedded `## Status: <value>` line (if present, else `pending`). Note: The `## Status:` line is optional and only relevant for manually-created task files or files generated by other tools. The standard T*.md template does not include it.
...
- **Post-import:** All tasks default to `phase: 0` with no phases mapping or critical path. Configure them after import:
  ```bash
...
**Behavior:**
- For task-level flags (`--status`, `--phase`, `--critical`, `--depends`, `--files`, `--specs`):
  - Validates task ID exists
